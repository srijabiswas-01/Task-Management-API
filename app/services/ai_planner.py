...
def generate_task_plan(
    ...
    for provider_name in settings.ai_provider_order:
        ...
        try:
            ...
            logger.info("AI task plan generated by %s in %.2fs", provider_name, time.monotonic() - started_at)
            return plan, provider_name, model, attempted > 1
        except (
            httpx.HTTPError,
            KeyError,
            IndexError,
            ...
            logger.warning(
                "AI task provider %s failed after %.2fs: %s",
                provider_name,
