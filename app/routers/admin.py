...
def organization_user(db: DB, current_user: User, user_id: int, *, with_profile: bool = False) -> User:
    """Resolve a user only inside the current administrator's organization."""
    query = select(User).where(User.id == user_id, User.organization_id == current_user.organization_id)
    if with_profile:
...
def organization_team(db: DB, current_user: User, team_id: int) -> Team:
    """Resolve a team without revealing cross-organization identifiers."""
    team = db.scalar(select(Team).where(Team.id == team_id, Team.organization_id == current_user.organization_id))
    if team is None:
...
def list_holidays(db: DB, current_user: CurrentUser) -> list[OrganizationHoliday]:
    require_system_admin(current_user)
    return list(db.scalars(select(OrganizationHoliday).where(
...
def create_holiday(payload: HolidayCreate, db: DB, current_user: CurrentUser) -> OrganizationHoliday:
    require_system_admin(current_user)
    if db.scalar(select(OrganizationHoliday.id).where(
        OrganizationHoliday.organization_id == current_user.organization_id,
        OrganizationHoliday.holiday_date == payload.holiday_date,
    )):
        raise HTTPException(status_code=409, detail="A holiday already exists on this date")
    item = OrganizationHoliday(organization_id=current_user.organization_id, name=payload.name.strip(), holiday_date=payload.holiday_date, description=payload.description, is_active=True)
...
def delete_holiday(holiday_id: int, db: DB, current_user: CurrentUser) -> None:
    require_system_admin(current_user)
    item = db.scalar(select(OrganizationHoliday).where(
        OrganizationHoliday.id == holiday_id,
        OrganizationHoliday.organization_id == current_user.organization_id,
    ))
...
def global_users(db: DB, organization_id: int, newest_first: bool = False) -> list[User]:
    ordering = (
    ...
    return list(db.scalars(
        ...
        .where(User.organization_id == organization_id).order_by(*ordering)
    ).all())
...
def list_global_users(db: DB, current_user: CurrentUser) -> list[UserDirectoryRead]:
    ...
    for user in global_users(db, current_user.organization_id, newest_first=True):
        percent, missing = profile_completion(user, user.profile)
...
def send_global_announcement(payload: GlobalAnnouncementSend, db: DB, current_user: CurrentUser) -> ProfileReminderResult:
    require_system_admin(current_user)
    query = select(User).where(User.organization_id == current_user.organization_id, User.is_active.is_(True), User.is_member.is_(True))
    ...
    if payload.audience == "all":
        conversation = db.scalar(select(ChatConversation).where(
            ChatConversation.organization_id == current_user.organization_id,
        ...
        if conversation is None:
            conversation = ChatConversation(organization_id=current_user.organization_id, workspace_id=None, scope_type="global", chat_type=ChatType.broadcast,
        ...
        for user in users:
            ...
            if conversation is None:
                conversation = ChatConversation(organization_id=current_user.organization_id, workspace_id=None, scope_type="direct", chat_type=ChatType.direct,
...
def assign_global_member(user_id: int, payload: GlobalMemberAssign, db: DB, current_user: CurrentUser) -> UserDirectoryRead:
    require_system_admin(current_user)
    user = organization_user(db, current_user, user_id, with_profile=True)
    if not user.is_active: raise HTTPException(status_code=404, detail="Active user not found")
    department = db.scalar(select(GlobalDepartment).where(GlobalDepartment.organization_id == current_user.organization_id, GlobalDepartment.name == payload.department.strip()))
    designation = db.scalar(select(GlobalDesignation).options(selectinload(GlobalDesignation.department)).where(GlobalDesignation.organization_id == current_user.organization_id, GlobalDesignation.name == payload.professional_title.strip()))
...
def update_global_user_access(user_id: int, is_active: bool, db: DB, current_user: CurrentUser) -> UserDirectoryRead:
    ...
    if user.is_system_admin and not is_active:
        active_admins = db.scalar(select(func.count(User.id)).where(User.organization_id == current_user.organization_id, User.is_system_admin.is_(True), User.is_active.is_(True)))
...
def get_global_user_profile(user_id: int, db: DB, current_user: CurrentUser) -> UserProfileRead:
    ...
    user = organization_user(db, current_user, user_id)
...
def delete_global_user(user_id: int, db: DB, current_user: CurrentUser) -> None:
    ...
    if user.is_system_admin:
        admins = db.scalar(select(func.count(User.id)).where(User.organization_id == current_user.organization_id, User.is_system_admin.is_(True)))
...
def global_skill_usage(db: DB, organization_id: int) -> dict[str, int]:
    usage: dict[str, int] = {}
    for value in db.scalars(select(UserProfile.skills).join(User).where(User.organization_id == organization_id)).all():
...
def list_global_skill_catalog(db: DB, current_user: CurrentUser) -> list[GlobalSkillRead]:
    """Return this organization's catalogue and safely import profile skills."""
    require_system_admin(current_user)
    existing = {item.name.casefold(): item for item in db.scalars(select(GlobalSkill).where(GlobalSkill.organization_id == current_user.organization_id)).all()}
    usage = global_skill_usage(db, current_user.organization_id)
    for value in db.scalars(select(UserProfile.skills).join(User).where(User.organization_id == current_user.organization_id)).all():
        for name in parse_skills(value):
            if name.casefold() not in existing:
                item = GlobalSkill(organization_id=current_user.organization_id, name=name)
                try:
                    # A second request may insert this skill after the initial
                    # catalogue query. Isolate that race to one savepoint.
                    with db.begin_nested():
                        db.add(item)
                        db.flush()
                    existing[name.casefold()] = item
                except IntegrityError:
                    duplicate = db.scalar(select(GlobalSkill).where(
                        GlobalSkill.organization_id == current_user.organization_id,
                        func.lower(GlobalSkill.name) == name.casefold(),
                    ))
                    if duplicate is not None:
                        existing[name.casefold()] = duplicate
    db.commit()
    items = list(db.scalars(select(GlobalSkill).where(GlobalSkill.organization_id == current_user.organization_id).order_by(func.lower(GlobalSkill.name))).all())
...
def create_global_skill(payload: GlobalSkillCreate, db: DB, current_user: CurrentUser) -> GlobalSkillRead:
    ...
    if db.scalar(select(GlobalSkill.id).where(GlobalSkill.organization_id == current_user.organization_id, func.lower(GlobalSkill.name) == name.casefold())):
        raise HTTPException(status_code=409, detail="A skill with this name already exists")
    item = GlobalSkill(organization_id=current_user.organization_id, name=name, description=payload.description.strip() if payload.description else None)
    db.add(item); db.commit(); db.refresh(item)
    return global_skill_response(item, global_skill_usage(db, current_user.organization_id))
...
def update_global_skill(skill_id: int, payload: GlobalSkillUpdate, db: DB, current_user: CurrentUser) -> GlobalSkillRead:
    require_system_admin(current_user)
    item = db.scalar(select(GlobalSkill).where(GlobalSkill.id == skill_id, GlobalSkill.organization_id == current_user.organization_id))
    if item is None: raise HTTPException(status_code=404, detail="Skill not found")
    name = " ".join(payload.name.split())
    duplicate = db.scalar(select(GlobalSkill.id).where(
        GlobalSkill.organization_id == current_user.organization_id,
        func.lower(GlobalSkill.name) == name.casefold(),
        GlobalSkill.id != skill_id,
    ))
    if duplicate: raise HTTPException(status_code=409, detail="A skill with this name already exists")
    old_key = item.name.casefold()
    for profile in db.scalars(select(UserProfile).join(User).where(User.organization_id == current_user.organization_id, UserProfile.skills.is_not(None))).all():
...
def list_global_skills(db: DB, current_user: CurrentUser) -> list[SkillMemberRead]:
    ...
    project_rows = db.execute(
        ...
        .where(Workspace.organization_id == current_user.organization_id).distinct()
    ).all()
    ...
    memberships = {item.user_id: item for item in db.scalars(
        select(GlobalTeamMember).join(Team).options(selectinload(GlobalTeamMember.team))
    ...
    users = [user for user in global_users(db, current_user.organization_id) if user.is_active and (user.is_member or user.is_system_admin)]
    result = []
    for user in users:
...
def team_member_analytics(db: DB, current_user: CurrentUser) -> dict:
    ...
    require_system_admin(current_user)
    users = [user for user in global_users(db, current_user.organization_id) if user.is_member or user.is_system_admin]
    memberships = list(db.scalars(select(GlobalTeamMember).options(
        selectinload(GlobalTeamMember.team)
    ).join(Team).where(Team.organization_id == current_user.organization_id)).all())
    ...
    ).where(Team.organization_id == current_user.organization_id).order_by(Team.name)).all())
    projects = list(db.scalars(select(Project).join(Workspace).options(selectinload(Project.workspace)).where(
        Workspace.organization_id == current_user.organization_id
    )).all())
    project_by_id = {item.id: item for item in projects}
    tasks = list(db.scalars(select(Task).join(Project).join(Workspace).options(
        selectinload(Task.task_assignees)
    ).where(Workspace.organization_id == current_user.organization_id)).all())
    ...
    rates = {item.name: item.hourly_rate for item in db.scalars(select(GlobalDesignation).where(
        GlobalDesignation.organization_id == current_user.organization_id
    )).all()}
    today = date.today()
    member_rows = []
...
def create_global_department(payload: DepartmentCreate, db: DB, current_user: CurrentUser) -> GlobalDepartment:
    require_system_admin(current_user)
    item = GlobalDepartment(organization_id=current_user.organization_id, name=payload.name.strip(), description=payload.description)
    db.add(item); db.commit(); db.refresh(item)
    return item
...
def list_global_designations(db: DB, current_user: CurrentUser) -> list[DesignationRead]:
    require_system_admin(current_user)
    items = db.scalars(select(GlobalDesignation).options(selectinload(GlobalDesignation.department)).where(
        GlobalDesignation.organization_id == current_user.organization_id
...
def create_global_designation(payload: DesignationCreate, db: DB, current_user: CurrentUser) -> DesignationRead:
    require_system_admin(current_user)
    if db.scalar(select(GlobalDepartment.id).where(GlobalDepartment.id == payload.department_id, GlobalDepartment.organization_id == current_user.organization_id)) is None: raise HTTPException(status_code=400, detail="Select a valid department")
    item = GlobalDesignation(organization_id=current_user.organization_id, name=payload.name.strip(), description=payload.description, department_id=payload.department_id, hourly_rate=payload.hourly_rate)
    db.add(item); db.commit(); db.refresh(item)
...
def global_team_manager(db: DB, user_id: int, organization_id: int) -> tuple[User, str]:
    user = db.scalar(select(User).options(selectinload(User.profile)).where(User.id == user_id, User.organization_id == organization_id, User.is_active.is_(True), User.is_member.is_(True)))
    ...
    designation = user.profile.professional_title if user.profile else None
    if not designation or db.scalar(select(GlobalDesignation.id).where(GlobalDesignation.organization_id == organization_id, GlobalDesignation.name == designation)) is None: raise HTTPException(status_code=400, detail="Assign the manager's department and designation first")
...
def list_global_teams(db: DB, current_user: CurrentUser) -> list[Team]:
    require_system_admin(current_user)
    return list(db.scalars(select(Team).options(selectinload(Team.manager_record).selectinload(TeamManager.user)).where(
...
def create_global_team(payload: TeamCreate, db: DB, current_user: CurrentUser) -> Team:
    require_system_admin(current_user); _, manager_designation = global_team_manager(db, payload.manager_user_id, current_user.organization_id)
    ensure_single_team_affiliation(db, payload.manager_user_id)
    team = Team(organization_id=current_user.organization_id, name=payload.name.strip(), description=payload.description, workspace_id=None)
...
def update_global_team(team_id: int, payload: TeamUpdate, db: DB, current_user: CurrentUser) -> Team:
    require_system_admin(current_user)
    team = organization_team(db, current_user, team_id)
    ...
    _, designation = global_team_manager(db, manager_user_id, current_user.organization_id)
...
def list_global_team_members(db: DB, current_user: CurrentUser) -> list[TeamMember]:
    require_system_admin(current_user)
    return list(db.scalars(select(TeamMember).join(Team).options(selectinload(TeamMember.user), selectinload(TeamMember.project)).where(Team.organization_id == current_user.organization_id).order_by(TeamMember.created_at)).all())
...
def add_global_team_member(team_id: int, payload: GlobalTeamMemberAdd, db: DB, current_user: CurrentUser) -> GlobalTeamMember:
    require_system_admin(current_user)
    team = organization_team(db, current_user, team_id)
    user = db.scalar(select(User).options(selectinload(User.profile)).where(User.id == payload.user_id, User.organization_id == current_user.organization_id, User.is_active.is_(True), User.is_member.is_(True)))
