...
function exportMemberReportPdf(report){const pages=[],s=report.summary,cover=pdfPageHeader(`${report.member.name} — Performance report`,`Personal progress and allocated-project performance`,1);cover.push(pdfRect(32,92,778,70,"#526dff"),pdfLabel(report.member.name,48,111,20,"#ffffff",true,500),pdfLabel(`${pretty(report.member.role)} · ${report.member.department||"No department"} · ${report.member.designation||"No designation"}`,48,139,9,"#e7eaff",false,600));[["Projects",s.projects],["Tasks",s.tasks],["Completed",s.completed],["Overdue",s.overdue],["Average progress",`${s.average_progress}%`],["Planned hours",s.planned_hours],["Remaining hours",s.remaining_hours],["Profile",`${report.member.completion_percent}%`]].forEach(([label,value],index)=>{const x=32+(index%4)*197,y=182+Math.floor(index/4)*62;cover.push(pdfRect(x,y,185,51,"#ffffff","#e4e9f1"),pdfLabel(label,x+10,y+9,7,"#71809c",true,160),pdfLabel(value,x+10,y+27,13,"#17233c",true,160))});cover.push(pdfLabel("Workload summary",32,330,12,"#17233c",true),...pdfWrapped(`${pretty(s.workload)} workload · ${s.planned_hours} active planned hours against ${report.member.weekly_capacity_hours} weekly capacity. Checklist completion: ${s.checklist_done}/${s.checklist_total}.`,32,353,760,9,"#526076",false,5,13));pages.push(cover);report.projects.forEach(project=>{const page=pdfPageHeader(project.name,`${project.workspace_name} · allocated-project performance`,pages.length+1);page.push(pdfRect(32,92,778,57,"#526dff"),pdfLabel(`${project.average_progress}% average progress · ${project.contribution_percent}% work contribution`,48,113,15,"#ffffff",true,720),pdfLabel(`${project.tasks} assigned tasks · ${project.completed} completed · ${project.overdue} overdue · ${project.planned_hours} planned hours`,48,136,8,"#e7eaff",false,720),pdfLabel("Assigned task timeline",32,175,11,"#17233c",true));report.tasks.filter(task=>task.project_id===project.id).slice(0,20).forEach((task,index)=>{const y=200+index*27;page.push(pdfRect(32,y,778,22,index%2?"#f8f9fc":"#ffffff","#e4e9f1"),pdfLabel(task.title,42,y+6,7,"#17233c",true,335),pdfLabel(`${pretty(task.status)} · ${task.progress}%`,390,y+6,7,"#526076",false,115),pdfLabel(`${task.planned_hours}h · ${task.due_date?date(task.due_date):"No due date"}`,535,y+6,7,"#526076",false,245))});pages.push(page)});downloadVisualPdf(pages,`${report.member.name.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}-performance-report.pdf`)}
